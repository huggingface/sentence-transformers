...
| Format | Columns | Loss |
|---|---|---|
| Pair | `anchor, positive` | `MultiVectorMultipleNegativesRankingLoss` |
| Triplet | `anchor, positive, negative` | `MultiVectorMultipleNegativesRankingLoss` |
| Multi-negative | `anchor, positive, negative_1, ..., negative_n` | `MultiVectorMultipleNegativesRankingLoss` |
| KD (per-row N candidates) | `query, documents, scores` where `documents` is a `list[str]` of size N and `scores` is a `list[float]` of size N | `MultiVectorDistillKLDivLoss` |
| Margin-MSE | `query, positive, negative` (single) or `query, positive, negative_1..k` with `label` of shape `(bs,)` or `(bs, k)` | `MultiVectorMarginMSELoss` |
...
See [`examples/multi_vector_encoder/training/`](https://github.com/UKPLab/sentence-transformers/tree/master/examples/multi_vector_encoder/training) for full
training scripts:
