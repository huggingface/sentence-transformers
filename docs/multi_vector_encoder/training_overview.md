...
The retrieval evaluators (:class:`~sentence_transformers.multi_vector_encoder.evaluation.MultiVectorInformationRetrievalEvaluator` and :class:`~sentence_transformers.multi_vector_encoder.evaluation.MultiVectorNanoBEIREvaluator`) can additionally report bootstrap confidence intervals over queries via ``bootstrap_resamples``, which adds ``_ci_low`` and ``_ci_high`` keys next to every metric. This helps judge whether a small difference between two checkpoints on a small benchmark like NanoBEIR (about 50 queries per dataset) is meaningful.
