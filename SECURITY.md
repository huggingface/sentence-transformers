...
- **Preferred:** GitHub private vulnerability reporting via the **"Report a vulnerability"** button under this repo's **Security** tab. This routes to the maintainers, keeps the report private until a fix is ready, and lets us issue a CVE through GitHub if warranted.
...
Exact released version or commit SHA you reproduced on (e.g. v5.6.0 / d138aef2).
Not "latest" or "main".

### Affected component
The public API, module, or entry point involved (e.g. `SentenceTransformer.encode`).
...
**Loading artifacts you did not create is a trust decision.** Models (including the module pipeline declared in their `modules.json` and any custom module code they ship), datasets, tokenizers, and configuration files can carry code or instructions that run when loaded. That a maliciously crafted artifact can execute code, read local files, or otherwise act when you load it is the **documented risk of loading untrusted content**, not a vulnerability in this library. Protect yourself by:
- preferring the `safetensors` format over pickle-based formats (`model_kwargs={"use_safetensors": True}`),
- pinning a specific, reviewed `revision`,
- setting `trust_remote_code=True` only for repositories whose custom code you have inspected.

We *will* treat as a vulnerability anything that breaks one of these protections, such as code executing despite `safetensors`-only loading, or a pinned revision being bypassed.
...
The following are **not** treated as vulnerabilities in Sentence Transformers. If your finding touches one of these, the report must explain why it is nonetheless a violation of a guarantee we make. Otherwise it will be closed.
...
- Module classes or executable code declared in a model's own files (module references in `modules.json`, custom module code shipped with the repository) and resolved when that model is loaded. Loading a repository that declares these is the same documented load-time trust decision, not a library flaw. (A control we advertise being bypassed, for example custom code from the Hub running without the `trust_remote_code` gate, remains in scope.)
...
- Model behavior: adversarial inputs that skew embeddings, similarity scores, or rankings (including documents crafted to rank highly in retrieval), embedding inversion, or membership inference. Model weights are authored by their uploaders, so report these to the model owner. Sentence Transformers produces embeddings and scores, not generated text or actions. There is no agent- or tool-execution code path here to inject into.
- Vulnerabilities in third-party dependencies we do not vendor (e.g. `torch`, `transformers`, `huggingface_hub`, `onnxruntime`). Report upstream (we'll bump once fixed).
